...
function getStatusColor(status: AttendanceStatus): string {
  ...
  return Colors[colorScheme][
    `status${status.charAt(0).toUpperCase() + status.slice(1).replace(/_./g, (m) => m[1].toUpperCase())}` as keyof typeof Colors.light
  ] as string;
}
...
function getPriorityMeta(attendance: Attendance): PriorityMeta {
  ...
  if (attendance.status === "completed") {
    return {
      label: "Histórico",
      helper: "Atendimento finalizado e preservado para análise.",
  ...
  if (attendance.status === "in_service") {
    return {
      label: "Alta atenção",
      helper: "Veículo em execução agora. Priorize o acompanhamento até a conclusão.",
  ...
  if (attendance.status === "waiting" && elapsedMinutes >= 30) {
    return {
      label: "Fila longa",
      helper: "Tempo de espera acima de 30 minutos. Vale revisar a fila.",
  ...
  if (attendance.status === "arrival" && elapsedMinutes <= 15) {
    return {
      label: "Entrada recente",
      helper: "Novo veículo no fluxo. Boa hora para triagem rápida.",
      backgroundColor: "rgba(0, 145, 234, 0.12)",
      textColor: "#005B9F",
    };
  }
  ...
  return {
    label: "Acompanhar",
    helper: "Fluxo normal do atendimento. Siga a evolução conforme o status.",
...
export function AdminAttendanceCard({
  ...
  const updatedLabel = new Date(Number(attendance.updatedAt)).toLocaleTimeString("pt-BR", {
    hour: "2-digit",
    minute: "2-digit",
  });
      ...
      <View style={styles.cardContent}>
        ...
        <View style={styles.infoRow}>
          <View style={styles.infoItem}>
            <ThemedText style={styles.infoLabel}>Tempo no fluxo</ThemedText>
            <ThemedText style={styles.infoValue}>{getElapsedTime(attendance.createdAt)}</ThemedText>
          </View>
          <View style={styles.infoItem}>
            <ThemedText style={styles.infoLabel}>Atualizado às</ThemedText>
            <ThemedText style={styles.infoValue}>{updatedLabel}</ThemedText>
          </View>
        </View>
