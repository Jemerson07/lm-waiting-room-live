import { StyleSheet, View, ScrollView, TextInput, Pressable, Switch, Alert, ActivityIndicator, Platform } from "react-native";
...
export default function SettingsScreen() {
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      ...
      await saveSettings({
        ...
        autoRefreshInterval: parseInt(autoRefreshInterval, 10) || 3,
      });
  ...
  const handleReset = () => {
    Alert.alert("Restaurar padrões", "Deseja restaurar as configurações padrão?", [
    ...
    return (
      <ThemedView style={[styles.container, { backgroundColor }]}> 
        <ScrollView contentContainerStyle={{ paddingTop: Math.max(insets.top, 20) + 20, paddingBottom: Math.max(insets.bottom, 20) + 20 }}>
          <View style={styles.header}><ThemedText type="title">Configurações</ThemedText><ThemedText style={styles.subtitle}>Área protegida para administração do sistema</ThemedText></View>
          <AccessRequiredCard title="Configurações protegidas" description="Faça login para editar dados da empresa, alertas e parâmetros do sistema." />
        </ScrollView>
      </ThemedView>
    );
  }
  ...
  return (
    <ThemedView style={[styles.container, { backgroundColor }]}>
      <ScrollView style={styles.scrollView} contentContainerStyle={[styles.scrollContent, { paddingTop: Math.max(insets.top, 20) + 20, paddingBottom: Math.max(insets.bottom, 20) + 20 }]}>
        ...
        <View style={[styles.section, { backgroundColor: cardBackground }]}>
          <ThemedText type="subtitle" style={styles.sectionTitle}>🔔 Notificações e Alertas</ThemedText>
          <View style={styles.settingRow}><View style={styles.settingLabel}><ThemedText style={styles.settingTitle}>Alertas Sonoros</ThemedText><ThemedText style={styles.settingDescription}>Toque ao atualizar status</ThemedText></View><Switch value={soundAlertsEnabled} onValueChange={(value) => { setSoundAlertsEnabled(value); setHasChanges(true); }} trackColor={{ false: "#ccc", true: tintColor }} thumbColor="#fff" /></View>
          <View style={styles.divider} />
          <View style={styles.settingRow}><View style={styles.settingLabel}><ThemedText style={styles.settingTitle}>Notificações</ThemedText><ThemedText style={styles.settingDescription}>Receber atualizações em tempo real</ThemedText></View><Switch value={notificationsEnabled} onValueChange={(value) => { setNotificationsEnabled(value); setHasChanges(true); }} trackColor={{ false: "#ccc", true: tintColor }} thumbColor="#fff" /></View>
          <View style={styles.divider} />
          <View style={[styles.infoBox, { backgroundColor: "rgba(0, 82, 163, 0.05)" }]}><ThemedText style={styles.infoText}>📱 WhatsApp: Notificações automáticas serão enviadas aos clientes quando o status do veículo mudar, desde que o número de telefone seja fornecido no formulário de atendimento.</ThemedText></View>
        </View>

        <View style={[styles.section, { backgroundColor: cardBackground }]}>
          <ThemedText type="subtitle" style={styles.sectionTitle}>⚙️ Sistema</ThemedText>
          <View style={styles.formGroup}><ThemedText style={styles.label}>Intervalo de Atualização (segundos)</ThemedText><TextInput style={[styles.input, { backgroundColor, borderColor }]} value={autoRefreshInterval} onChangeText={(text) => { setAutoRefreshInterval(text); setHasChanges(true); }} placeholder="3" placeholderTextColor="#999" keyboardType="number-pad" /><ThemedText style={styles.helperText}>Frequência de sincronização com o servidor (mínimo: 1 segundo)</ThemedText></View>
        </View>
        ...
        <View style={styles.actionButtons}>
          <Pressable style={[styles.button, styles.resetButton, { borderColor: "#FF3B30" }]} onPress={handleReset}><ThemedText style={[styles.buttonText, { color: "#FF3B30" }]}>Restaurar Padrões</ThemedText></Pressable>
          <Pressable style={[styles.button, styles.saveButton, { backgroundColor: tintColor }, !hasChanges && styles.buttonDisabled]} onPress={handleSaveSettings} disabled={!hasChanges || saving}>{saving ? <ActivityIndicator color="#fff" /> : <ThemedText style={styles.buttonText}>{hasChanges ? "💾 Salvar Alterações" : "✓ Salvo"}</ThemedText>}</Pressable>
        </View>
      </ScrollView>
    </ThemedView>
  );
...
const styles = StyleSheet.create({
  ...
  button: { paddingVertical: 14, borderRadius: 8, alignItems: "center", minHeight: 48, justifyContent: "center" },
  resetButton: { borderWidth: 2, backgroundColor: "transparent" },
  saveButton: { elevation: 4, shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.15, shadowRadius: 4 },
