...
import { AttachmentGallery } from "@/components/maintenance/attachment-gallery";
...
export default function OrderMediaSafeScreen() {
    ...
    <ThemedView style={styles.container}>
      <ScrollView style={styles.scroll} contentContainerStyle={{ paddingTop: Math.max(insets.top, 20) + 16, paddingBottom: 32 }}>
        <View style={styles.hero}>
          <ThemedText type="title" style={styles.heroTitle}>Mídias da manutenção</ThemedText>
        ...
        <View style={styles.card}>
          <ThemedText type="subtitle">Pré-visualização das evidências</ThemedText>
          <ThemedText style={styles.subtle}>Imagens exibem miniatura. PDFs e arquivos mostram cartão de acesso rápido.</ThemedText>
          {loadingAttachments ? (
            <ActivityIndicator />
          ) : attachmentsError ? (
